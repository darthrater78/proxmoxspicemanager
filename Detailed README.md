...
The resulting `dist\Proxmox SPICE Manager.exe` bundles Python and tkinter into a single file. virt-viewer must still be installed separately (the app will find `remote-viewer.exe` automatically via PATH, common install directories, or the Windows registry).

Use **File → Create Start Menu Shortcut** inside the app to pin it to your Start Menu.
